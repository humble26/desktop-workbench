...
function createAiMonitor(deps) {
   ...
   * @returns {Promise<{ok:boolean, refreshed:number, summary:object}>}
   */
  async function refresh(ids) {
    ...
    try {
      const s = settings();
      let list = Array.isArray(ids) && ids.length ? ids.map(x => String(x).toLowerCase()) : enabledIds(s);
      list = list.filter(id => providers.providerById(id));
      ...
      return { ok: true, refreshed: list.length, summary: sum };
    } finally {
      refreshing = false;
    }
  }
