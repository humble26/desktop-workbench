...
function createStore(opts) {
  ...
  let lastFatal = false;        // 主文件与全部备份均无法读取，已用默认值重建（B10）
  ...
  function load() {
    ...
    if (!parsed) {
      ...
      if (!parsed && existed) {
        // 主文件与全部备份都读不出来：只能用默认值重建（审查 B10 —— 此前完全
        // 无痕，用户数据全损却毫无痕迹）。坏文件改名留存（*.corrupt-<时间戳>），
        // 既保住原始现场供找回，也避免默认值直接覆盖它；诊断侧据此提示用户。
        lastFatal = true;
        try { fs.renameSync(filePath, filePath + '.corrupt-' + stamp()); } catch (e) { /* ignore */ }
      }
    }
    ...
    /* 只在「确实需要」时落盘：首次运行建立文件、从备份恢复、致命重置、或结构有改动。
       内容没变就不写，避免每次启动都重写数据文件。 */
    if (isNewFile || lastRecovery || lastFatal || result.changed) {
      dirty = true;
      writeNow();
    }
    return data;
  ...
  // ---------------------------------------------------------------- 变更入口
  /* silent=true 时只推进修订号并落盘、不向外 emit（用于高频/低价值的
     变更，如窗口位置记忆——广播会触发渲染层整页重渲染，得不偿失）。 */
  function bump(origin, changed, silent) {
    if (!changed) return { ok: true, rev: rev, changed: false };
    rev++;
    scheduleWrite();
    if (!silent) emit({ rev: rev, origin: origin || 'main' });
  ...
  function mutate(fn, opts2) {
    ...
    const r = bump((opts2 && opts2.origin) || 'main', changed, opts2 && opts2.silent);
  ...
  /* 接受调用方持有并已就地修改过的权威对象（历史 saveStore(data) 写法）。
     ...
     opts.silent=true 时不广播（见 bump 说明）。
     ⚠️ 这里刻意**不做** deepEqual 来省掉「无变化」的 bump（审查 B9 的结论更正）：
     主流调用形如 `const d = loadStore(); d.settings.x = …; saveStore(d);` ——
     传入的就是内部权威引用本身（已被就地修改），deepEqual(d, d) 恒等，
     会连真实改动一起跳过导致**永不落盘**。担心空转 rev 的调用方应自行
     比较后再决定是否调用（saveWinBounds 即此模式）。 */
  function adopt(next, opts) {
    const d = read();
    if (isObj(next) && next !== d) data = next;
    return bump('main', true, opts && opts.silent);
  }
