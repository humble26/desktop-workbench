...
const { normalizeAiSettings } = require('./ai/settings.js');
...
function normalizeCollections(data, mark) {
  ...
  for (const s of data.shortcuts) {
    // 图标引用只接受本应用缓存产物（file:// 短引用 / icon: 引用）；
    // 旧式 base64 内联与历史占位串清空 —— 判据与渲染层 app.js 共用 proto.isInlineIcon
    if (proto.isInlineIcon(s.icon)) mark.set(s, 'icon', null);
  }
}
...
function normalizeSettings(data, mark) {
  ...
  // AI 余额监测：与 lib/ai/settings.js 共用同一份归一化实现。
  // 这里用 JSON 快照对比来判断「是否真的改了」，而不是逐字段计数 ——
  // 归一化是幂等的，因此不会出现「每次启动都重写文件」的问题；
  // normalizeAiSettings 保留未知字段，所以补齐默认值不会删用户配置。
  const aiBefore = JSON.stringify(s.aiMonitor === undefined ? null : s.aiMonitor);
  s.aiMonitor = normalizeAiSettings(s.aiMonitor);
  if (JSON.stringify(s.aiMonitor) !== aiBefore) mark.n++;
