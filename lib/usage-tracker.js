...
function createUsageTracker(deps) {
  ...
  let usageLastSaveError = null;  // 最近一次落盘失败原因（诊断页展示；审查 B14——静默丢失不可见）
  ...
  function saveUsageData() {
    try {
      ...
      usageLastSaveError = null;
    } catch (e) {
      usageLastSaveError = String((e && e.message) || e);   // 不抛出（采样不能被落盘故障打断），但必须可诊断
    }
  }
  ...
  return {
    ...
    /** 最近一次 usage-data.json 落盘失败原因；成功后清为 null（诊断页展示） */
    lastSaveError: () => usageLastSaveError
  };
}
